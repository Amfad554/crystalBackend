...
exports.addEquipment = async (req, res) => {
  try {
    ...
    // 1. Defensively handle the image
    let imageUrl = null;
    if (req.file) {
      // If using your custom utility:
      imageUrl = await uploadToCloudinary(req.file.buffer, "image", "equipment");
    }

    // 2. Prevent "NaN" or empty string crashes for numeric fields
    const parsedRate = (dailyRate && !isNaN(dailyRate)) ? parseFloat(dailyRate) : 0;
    ...
    console.error("CRASH LOG:", error); // This shows in your Render Logs
    res.status(500).json({ success: false, message: "Server error: Check data types" });
  }
};
